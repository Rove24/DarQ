...
import com.kieronquinn.app.darq.utils.extensions.findSwitch
...
class SettingsFragment : BaseSettingsFragment<FragmentSettingsBinding>(FragmentSettingsBinding::inflate), AutoExpandOnRotate {
    ...
    override val settingsItems by lazy {
        listOf(
            ...
            SettingsItem.SwitchSetting(
                ...
                rowClickAction = viewModel::onAutoDarkClicked
            ),
            SettingsItem.Setting(
    ...
    private fun applyMainSwitchStyle() = with(binding.switchMain.mainSwitchSwitch) {
        background = ContextCompat.getDrawable(requireContext(), R.drawable.bg_group_card_single)
        backgroundTintList = null
        setTextColor(ContextCompat.getColor(requireContext(), R.color.switch_main_text))
        findSwitch()?.let { sw ->
            sw.trackDrawable = ContextCompat.getDrawable(requireContext(), R.drawable.md3_switch_track)
            sw.thumbDrawable = ContextCompat.getDrawable(requireContext(), R.drawable.md3_switch_thumb)
            sw.applyMD3SwitchMonet()
        }
    }
    ...
    private fun setupMainSwitch() {
        binding.switchMain.run {
            ...
            applyMainSwitchStyle()
            mainSwitchSwitch.post {
                applyMainSwitchStyle()
            }
            updateMainSwitch()
        }
